...
LA PLATA, 23 DE MARZO 2021 HOJA: 6 de `
    ...
    while (next) {
        ...
        if (inicio === -1) {
            ...
            let PDO=datItem.substr(datItem.indexOf("PDO:")+4, ( datItem.indexOf("PDO:") - datItem.indexOf("PDO:")+4) ).trim()
            let PARTIDA = datItem.substr(datItem.indexOf("PARTIDA:")+8, ( datItem.indexOf("**NOM") - datItem.indexOf("PARTIDA:")+8) ).replace(/\n|:/g,'*').split('*')[0].trim()
            let C=datItem.substr(datItem.indexOf("C:")+2, ( datItem.indexOf("S:") - datItem.indexOf("C:")-2) ).trim()
            let S=datItem.substr(datItem.indexOf("S:")+2, ( datItem.indexOf("MZ:") - datItem.indexOf("S:"))-2).trim()
            let SP=datItem.substr(datItem.indexOf("SP:")+3, ( datItem.indexOf(".DOMINIO") - datItem.indexOf("SP:"))-0).replace(/\n/g,'*').split('*')[0]
            let ID_PDO = datItem.substr(datItem.indexOf("INSCRIPC."), ( datItem.indexOf("TI:") - datItem.indexOf("INSCRIPC."))-0).split(':')[2].trim()
            let TI=datItem.substr(datItem.indexOf("TI:"), ( datItem.indexOf("NRO:") - datItem.indexOf("TI:"))-0).split(':')[1].trim()
            let NRO = datItem.substr(datItem.indexOf("NRO:"), ( datItem.indexOf("UF:") - datItem.indexOf("NRO:"))-0).split(':')[1].trim()
            let UF = datItem.substr(datItem.indexOf("UF:"), ( datItem.indexOf("AÑO:") - datItem.indexOf("UF:"))-0).split(':')[1].trim()
            let AÑO=datItem.substr(datItem.indexOf("AÑO:"), ( datItem.indexOf("S/Z:") - datItem.indexOf("AÑO:"))-0).split(':')[1].trim()
            let SZ = datItem.substr(datItem.indexOf("S/Z:"), ( datItem.indexOf("NAC:") - datItem.indexOf("S/Z:"))-0).replace(/\n/g,'*').split('*')[0].split(':')[1].trim()
            
            //console.log({i,a:datItem.substr(datItem.indexOf("F.NAC:"), ( datItem.indexOf("ORGANISMO:") - datItem.indexOf("F.NAC:"))-0)})
            let NAC=''
            let CUIT=''
            let DNI=datItem.substr(datItem.indexOf("DOC:"), ( datItem.indexOf("CUIT:") - datItem.indexOf("DOC:"))-0).split(':')[1].trim()
            //console.log({i,a:datItem.substr(datItem.indexOf("DOMICILIO:"), ( datItem.indexOf("P.I.:") - datItem.indexOf("DOMICILIO:"))-0)})
            let ORGANISMO=''
            let DOMICILIO=''
            let LOCALIDAD=''
            let PROVINCIA=''
            let PI=datItem.substr(datItem.indexOf("P.I.:"), 10).replace(/\n/g,'*').split('*')[0].split(':')[1].trim()

            //console.log({i,a:datItem.substr(datItem.indexOf("P.I.:"), 10).replace(/\n/g,'*').split('*')[0].split(':')[1].trim()})
            ...
            items.push({
                ...
                partido:PDO,
                partida:PARTIDA,
                nom_cat:{   //Nomenclatura catastral
                    c:C,
                    s:S,
                    mz:MZ, 
                    pa:PA,
...
let DATA = items
let fileName ='terrenos-'+name.toLowerCase().replace(' ','-').replace(' ','-').replace(' ','-').replace(' ','-')
